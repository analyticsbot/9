...
def postToWP(title, article, url_bing, bing_image_list):
...
def insertImages(profile, imagename):
    ...
    rows = c.fetchone()
    if rows is None:
        create_table_query = 'CREATE TABLE ' + str(profile) + '_images ( ID INTEGER PRIMARY KEY AUTOINCREMENT, image CHAR(500000));'
        c.execute(create_table_query)
        conn.commit()
        for url in urls_to_scrape:
            insert_url_query = 'INSERT INTO ' + str(profile) + 'images (URL, SCRAPED)  VALUES ("' + str(url) +'",0);'
            c.execute(insert_url_query)
        conn.commit()
...
for i in range(atOnce):
    ...
    #status, bing_image_list = postToWP(title, spinText, url_bing, bing_image_list)
