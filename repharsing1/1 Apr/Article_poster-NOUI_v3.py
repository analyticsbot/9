...
import sys, sqlite3, time, argparse, textwrap, random
from newspaper import Article
try:
    import urllib.request as urllib2
except ImportError:
    import urllib2
import json, shutil, requests, logging, time, random, feedparser, base64, codecs
...
searchString = '%27'+args.keyword+'%27'
#searchString = '%27' + args.keyword + '%27'
top = 20
offset = 0
...
def getGoodURLs(include, exclude, urls, titles):
    return_urls = []
    if len(include)>2 and len(exclude)>2:    
        for url in urls:
            ix = urls.index(url)
            title_ = titles[ix]
            if ((not (any(word in url for word in exclude))) or (not (any(word in title_.lower() for word in exclude))))\
                or (((any(word in url for word in include))) or ((any(word in title_.lower() for word in include)))):
                return_urls.append(url)
                
    elif len(include)>0 and len(exclude)==1:
        for url in urls:
            ix = urls.index(url)
            title_ = titles[ix]
    ...
    elif len(include)==1 and len(exclude)>0:
        for url in urls:
            ix = urls.index(url)
            title_ = titles[ix]
            if (not (any(word in url for word in exclude))) or (not (any(word in title_.lower() for word in exclude))):
...
exclude = 'forbes, washingtonpost, coupon, review, offer, sale, deal, buy,cgi-sys, free, accessoriesfreeads, discount, coupons, podcast, radio, subscribe, quickbooks, fuck'
...
spinner = 'wordai'
urls = ['https://flipboard.com/topic/' + args.topic + '.rss']
