...
class AmazonSpider(Spider):
    ...
    def start_requests(self):
        ...
        for st in self.searchterms:
            self.log("Scrapping %s search term." % st, INFO)
            yield Request(
    ...
    def _get_products(self, response):
        ...
        if total_matches is None:
            self.log("total_matches is none", INFO)
    ...
    def _scrape_product_links(self, response):
        products = response.xpath('//li[@class="s-result-item  celwidget "]')
        ...
        for pr in products:
            ...
            # check whether this is the asin when the spider has to stop
            if pr.xpath('@data-asin').extract()[0] == self.asin:
                self.log('ASIN found. Stopping the spider', INFO)
                raise CloseSpider('ASIN %s found' % self.asin)
